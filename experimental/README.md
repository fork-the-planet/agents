...
- **[workspace-chat](./workspace-chat/)** — AIChatAgent with a persistent virtual filesystem. Demonstrates `Workspace` from `@cloudflare/shell` with file tools and sandboxed `state.*` execution via `@cloudflare/codemode`.
